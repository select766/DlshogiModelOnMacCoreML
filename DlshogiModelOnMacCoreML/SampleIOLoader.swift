...
func getSampleIO(batchSize: Int) -> SampleIO {
    ...
    guard let url = Bundle.main.url(forResource: "SampleIO15x224MyData", withExtension: "bin") else {
        fatalError("SampleIO15x224.bin not found")
    }
    guard let data = try? Data(contentsOf: url) else {
        fatalError("Failed to read SampleIO15x224.bin")
    }
