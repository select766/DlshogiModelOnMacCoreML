...
class ModelRunner : Thread {
    ...
    init(model: DlShogiResnet15x224SwishBatch, batchSize: Int, loadedModelComputeUnits: String, duration: TimeInterval, updateMessage: @escaping (String) -> Void) {
        self.model = model
    ...
    func runModel() {
        ...
        var pred: DlShogiResnet15x224SwishBatchOutput?
        var timeNow = Date()
        var sampleCount = 0
